...
import { Eye, EyeOff, TicketCheck, User, Shield, ArrowLeft } from "lucide-react";
import axios from "axios";

export default function Signup() {
  const [formData, setFormData] = useState({ email: "", password: "", confirmPassword: "", fullName: "", role: "student" });
    ...
    try {
      ...
      try {
        ...
        await axios.post(`${API_URL}/api/auth/register`,
          { email: formData.email, full_name: formData.fullName, role: formData.role },
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } catch (err) {
        console.warn("Backend registration failed:", err.message);
      }
      localStorage.setItem(`user_role_${cred.user.uid}`, formData.role);
    } catch (err) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg w-full max-w-sm px-8 py-10 relative">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1.5 py-3 rounded-xl border-2 transition text-sm font-semibold ${formData.role === value ? active : inactive}`}
              >
                <Icon size={20} />
                {label}
              </button>
          ...
          <button
            ...
            className="w-full bg-teal-500 hover:bg-teal-600 text-white font-semibold py-2.5 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed text-sm tracking-wide"
          >
